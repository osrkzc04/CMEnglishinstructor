...
import { StaffFormSchema, type StaffFormInput } from "@/modules/users/schemas"
import { updateStaffUser } from "@/modules/users/update.action"
...
export function StaffPersonalDataForm({ userId, initialValues, isSelf }: Props) {
  ...
  const onSubmit = handleSubmit((data) => {
    ...
    startTransition(async () => {
      const result = await updateStaffUser(userId, data)
      if (!result.success) {
        if (result.field) {
    ...
    <form onSubmit={onSubmit} noValidate className="space-y-5">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <Field id="role" label="Rol" error={errors.role?.message}>
          ...
          {isSelf && (
            <p className="text-text-3 mt-1 text-[12px]">No puedes cambiar tu propio rol.</p>
          )}
        </Field>
        <Field id="status" label="Estado" error={errors.status?.message}>
          ...
          >
            ...
            <option value={UserStatus.PENDING_APPROVAL}>Pendiente</option>
