...
export function MarketingDifferentiators() {
  return (
    <section id="por-que" className="border-border bg-surface-alt/40 border-t py-20 md:py-28">
      <div className="mx-auto w-full max-w-[1180px] px-6">
        <Reveal as="header" className="mb-14 max-w-[720px]">
          ...
          <h2 className="text-foreground font-serif text-[36px] leading-[1.12] tracking-[-0.02em] sm:text-[44px]">
            ...
            <em className="text-text-2 font-light italic"> en serio</em>.
          </h2>
          <p className="text-text-2 mt-4 text-[16px] leading-[1.65]">
            Cuatro razones por las que estudiantes y empresas eligen seguir con nosotros nivel tras
            nivel.
          </p>
        </Reveal>
        ...
        <div className="grid gap-x-10 gap-y-8 md:grid-cols-2">
          {DIFFERENTIATORS.map((d, i) => (
            <Reveal key={d.number} delay={i * 80}>
              <article className="group border-border flex gap-5 border-t pt-6 transition-colors duration-300 hover:border-teal-500/40">
                <span className="text-text-3 font-serif text-[34px] leading-none font-light tracking-[-0.02em] transition-colors duration-300 group-hover:text-teal-500 sm:text-[40px]">
                  {d.number}
                </span>
                <div>
                  <h3 className="text-foreground font-serif text-[22px] leading-[1.2] tracking-[-0.015em] sm:text-[24px]">
                    {d.title}
                  </h3>
                  <p className="text-text-2 mt-2.5 text-[15px] leading-[1.6]">{d.description}</p>
                </div>
              </article>
            </Reveal>
          ))}
