...
export function MarketingEnterprise() {
  return (
    <section
      id="empresas"
      className="relative overflow-hidden border-y border-border bg-surface-alt/40 py-20 md:py-28"
    >
      <EnterpriseBackdrop />
      ...
      <div className="relative mx-auto grid w-full max-w-[1180px] gap-14 px-6 lg:grid-cols-[1fr_1.1fr]">
        <Reveal>
          ...
          <h2 className="font-serif text-[36px] leading-[1.12] tracking-[-0.02em] text-foreground sm:text-[44px]">
            ...
            <em className="font-light italic text-text-2"> sin fricción</em>.
          </h2>
          <p className="mt-5 max-w-[480px] text-[16px] leading-[1.65] text-text-2">
            Trabajamos con bancos, consultoras, tecnológicas y equipos de
            operaciones. Diseñamos un plan de inglés ejecutivo alineado al
            negocio, con visibilidad continua para RRHH y facturación
            corporativa.
          </p>

          <a
            href="mailto:hola@​cmenglishinstructor.com?subject=Propuesta%20corporativa"
            className="group mt-8 inline-flex items-center gap-2 rounded-md bg-ink-900 px-5 py-3 text-[14px] font-medium text-bone transition-colors hover:bg-teal-500 dark:bg-bone dark:text-ink-900 dark:hover:bg-teal-500 dark:hover:text-bone"
          >
            Solicitar propuesta
            <ArrowRight
              size={15}
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          {CAPABILITIES.map((c, i) => (
            <Reveal key={c.title} delay={i * 70}>
              <article className="group flex h-full flex-col rounded-xl border border-border bg-surface/95 p-5 backdrop-blur-sm transition-[border-color,transform] duration-300 ease-out hover:-translate-y-0.5 hover:border-teal-500/40 motion-reduce:transition-none motion-reduce:transform-none">
                <div className="mb-4 inline-flex h-9 w-9 items-center justify-center rounded-full border border-border bg-background text-foreground transition-colors duration-200 group-hover:border-teal-500/40 group-hover:text-teal-500 motion-reduce:transition-none">
    ...
    <div aria-hidden className="pointer-events-none absolute inset-0">
      <div className="absolute inset-0 overflow-hidden">
        <Image
          ...
          fill
          sizes="100vw"
          className="object-cover opacity-[0.18] [filter:grayscale(0.85)_contrast(1.05)] mix-blend-multiply dark:opacity-[0.12] dark:mix-blend-screen"
        />
        ...
        <div className="absolute inset-x-0 top-0 h-24 bg-gradient-to-b from-surface-alt to-transparent" />
